...
def translate(sentence: str):
    ...
    if(type(sentence) == int or sentence.isdigit()):
        id = int(sentence)
        ds = load_dataset(config['datasource'], split='test')
        ds = BilingualDataset(ds, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'], config['seq_len'])
    ...
    with torch.no_grad():
        ...
        source = torch.cat([
            ...
            torch.tensor([tokenizer_src.token_to_id('[PAD]')], dtype=torch.int64)
        ], dim=0).to(device)
